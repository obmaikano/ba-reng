...
# STATE.md's "Checkpoint:" field commonly carries a human title (and sometimes a
# trailing [UI] tag) alongside the bare id, e.g. "4.6 — Dispute Resolution Center"
# or "4.6 — Dispute Resolution Center [UI]". Recover the bare checkpoint id from
# such a value so it can be compared to the bare ids parsed from PLAN headings.
_LEADING_CHECKPOINT_ID_RE = re.compile(rf"(?<![\w.])({CHECKPOINT_ID_PATTERN})(?![\w])")


def _normalize_cid_safe(value: str) -> str:
    try:
...
def _extract_checkpoint_id(raw: str | None) -> str | None:
    """Return the bare checkpoint id (e.g. "4.6", "4A.0") at the start of a
    STATE.md "Checkpoint:" value, or None if none can be recovered."""
    if not raw:
        return None
    m = _LEADING_CHECKPOINT_ID_RE.search(raw)
    return m.group(1) if m else (raw.strip() or None)


def _next_checkpoint_after(plan_ids: list[str], current_id: str) -> str | None:
    # STATE.md may hold the checkpoint as "4.6 — Title" rather than bare "4.6",
    # so normalise before comparing against the bare ids parsed from PLAN.md.
    # Compare on normalised forms but return the original heading-form id so
    # downstream stage detection / dependency checks see the canonical id.
    needle = _normalize_cid_safe(_extract_checkpoint_id(current_id) or current_id)
    norm_ids = [_normalize_cid_safe(cid) for cid in plan_ids]
    try:
        idx = norm_ids.index(needle)
    except ValueError:
...
def _extract_checkpoint_section(plan_text: str, checkpoint_id: str) -> str | None:
    ...
    """
    indexed_lines = list(_iter_visible_markdown_lines(plan_text, keepends=True))
    # \b only matches between a word char and a non-word char, so it can
    # never match immediately after a checkpoint_id that itself ends in a
    # non-word character (e.g. a title tagged "... [UI]" ending in "]") —
    # that made every bracket-tagged checkpoint heading unmatchable. Only
    # require the boundary on whichever end is actually a word character.
    leading_boundary = r"\b" if re.match(r"^\w", checkpoint_id) else ""
    trailing_boundary = r"\b" if re.search(r"\w$", checkpoint_id) else ""
    head_pat = re.compile(
        r"^\s*(#{3,6})\s+.*?"
        + leading_boundary
        + re.escape(checkpoint_id)
        + trailing_boundary
        + r".*?$"
    )
...
class _DecisionContext:
    ...
    product_validation_passed_for_current_state: bool
...
def _product_validation_passed_for_current_state(repo_root: Path) -> bool:
    """True when a product_validation PASS is the latest acknowledged loop result.

    Product validation sits between implementation and technical review. A PASS
    does not move status off `IN_REVIEW`; instead it is recorded in LOOP_RESULT.json
    and tied to the current STATE snapshot via `state_sha256`. The dispatcher reads
    that record to decide whether to advance to technical review (`review`) or to
    (re)run product validation. Mirrors `_recent_resolved_triage_for_current_state`.
    """
    payload, error = _load_loop_result_record(repo_root)
    if error or not isinstance(payload, dict):
        return False
    recorded_hash = str(payload.get("state_sha256", "")).strip()
    if not recorded_hash or recorded_hash != _state_sha256(repo_root):
        return False
    if str(payload.get("loop", "")).strip() != "product_validation":
        return False
    return str(payload.get("result", "")).strip().lower() == "pass"
...
def _gather_decision_context(state: StateInfo, repo_root: Path) -> _DecisionContext:
    ...
    return _DecisionContext(
        ...
        product_validation_passed_for_current_state=_product_validation_passed_for_current_state(repo_root),
...
def _decide_role(state: StateInfo, ctx: _DecisionContext) -> tuple[Role, str, str | None]:
    ...
    if state.status == "IN_REVIEW":
        ...
        # Product validation (UAT) gates technical review. A PASS does not move
        # status off IN_REVIEW; it is recorded in LOOP_RESULT.json and tied to the
        # current STATE snapshot, so the dispatcher advances to `review` exactly
        # once per (state, checkpoint). FAIL reverts status to IN_PROGRESS/BLOCKED.
        if ctx.product_validation_passed_for_current_state:
            return ("review", "Product validation PASS recorded; technical review can proceed.", None)
        return (
            "product_validation",
            "Checkpoint status is IN_REVIEW; run product validation (UAT) before technical review.",
            None,
        )
...
PROMPT_MAP: dict[Role, dict[str, str]] = {
    ...
    "product_validation": {
        "id": "prompt.product_validation",
        "title": "Product Validation & UAT Prompt",
    },
    "implement": {
