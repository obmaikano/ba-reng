...
- **Backend tests MUST be executed** via `docker run --rm --network host -v "$PWD/ohma-backend:/app" -w /app golang:1.24 go test ./... -p 1 -count=1`. "Constructed but not compiled" tests are strictly forbidden; if Docker Go tests cannot run, set status to BLOCKED.
- **Mobile apps MUST be compiled and launched on the Android emulator** for `[UI]` checkpoints. `tsc --noEmit` alone is not sufficient evidence of readiness.
- **Any new or modified SQL query MUST be covered by a Go test** that executes against the live PostgreSQL/PostGIS Docker container, proving the columns and tables actually exist. `go build`/`go vet` do not catch schema mismatches.
...
7) Evaluator-Optimizer pass (required):
   ...
   - If any score < 5, run one targeted repair pass and rescore once.
   - If any score is still < 5, set status to `IN_PROGRESS|BLOCKED` and route to `issues_triage`.
   - `evidence_quality: 4` (e.g. emulator gap) is no longer acceptable now that an emulator is available.
...
## prompt.product_validation — Product Validation & UAT Prompt

```md
ROLE: Product Manager (User Acceptance Testing & Scope Guardian)

GOAL
Conduct a validation session where user stories are explicitly asked and answered.
Verify that the implementation delivers real user value under a zero-bug MVP policy
and strictly adheres to scope *before* technical code review begins. This loop sits
between implementation and technical review: it asks "Did we build the right thing
for the user?" before review asks "Is the code technically correct?"

PREFLIGHT
1) Read `AGENTS.md` (optional if already read), `.vibe/STATE.md`, `.vibe/PLAN.md`.
2) Read the relevant Design Authority / Research docs for the current checkpoint
   (e.g. `docs/flow-research-*.md`, `docs/PRODUCT-CAPABILITY.md`).
3) Review the implementation work log and evidence from the `IN_REVIEW` state.
4) Confirm scope: this is a product/scope gate, not a code-correctness or
   adversarial review — those belong to the `review` loop that runs next.

ALLOWED FILES
- `.vibe/STATE.md` (for work-log notes and issue creation only)
- `.vibe/PLAN.md` (read-only)
- Test/log/screenshot/curl output files produced by verification commands

REQUIRED STATE MUTATIONS
- If ANY user story FAILS: Set status to `IN_PROGRESS` or `BLOCKED`. Add an
  `ACTIVE ISSUE BLOCK` detailing the failed story and the exact rework needed.
- If ALL user stories PASS: Add a work-log entry noting "Product Validation: PASS"
  (or "UAT Session: PASS") and proceed to handoff. **Do not change status from
  `IN_REVIEW`** — the dispatcher reads this loop's recorded PASS to advance to
  technical review; changing status here would bypass the `review` loop.
...
EXECUTION
1) **Formulate User Stories:**
   Based on the checkpoint Objective and Deliverables in `PLAN.md`, write 2–3
   explicit User Stories in the format:
   "As a [Role], I want to [Action] so that [Benefit]."
   (Roles are product roles — e.g. sender, driver, admin — not code components.)

2) **Conduct the Validation Session (The "Ask & Answer" Phase):**
   For each user story, act it out against the live system and explicitly answer it.
   Binary verdicts only — explicit YES/NO with immediate proof.
   - **For `[UI]` checkpoints:** You MUST use the Android emulator
     (e.g. `cd ohma-mobile/apps/sender && npx expo run:android` or
     `.../apps/driver...`). Walk through the exact UI flow. Take screenshots or
     describe the exact visual state at each step.
   - **For backend checkpoints:** You MUST execute `curl` against the live Docker
     backend to simulate the user action and inspect the JSON response; confirm it
     provides the exact data the frontend needs to render the experience.
   - **Zero-Bug Policy:** If the user encounters a crash, a confusing empty state, a
     broken or missing API field, a broken flow, or a design-lock violation, the
     story FAILS. Do not accept `tsc --noEmit` / `go build` / a green test suite as
     proof of user value.

3) **Scope Discipline (The "Creep" Test):**
   - Compare deliverables in `PLAN.md` against the actual implementation.
   - FAIL if the agent built features not requested in the checkpoint (gold-plating).
   - FAIL if the agent skipped a deliverable to save time.
   - MVP means minimal viable: no premature optimisations or unrequested features.

4) **Design Lock Adherence:**
   - If a Design Authority was named for the checkpoint, does the implementation
     match the locked traits (colors, roles, density, affordances)?
   - FAIL if semantic rules are broken (e.g. red used for confirm buttons).

5) **Answer Formatting (strict Q&A):**
   Output each story as:
   - **Story N:** "As a [role], I want to ... so that ..."
   - **Asked:** <one concrete question, answerable YES/NO against the live system>
   - **Answered:** YES|NO — <screenshot reference / curl output / observed flow>

REQUIRED OUTPUT
A) Verdict: PASS | FAIL
B) UAT Session Results (Stories, Asked, Answered, Evidence)
C) Scope Discipline Assessment (any creep or missing pieces?)
D) Issues created/updated (if any story failed)
E) State transition applied (work-log note added; status unchanged on PASS)

REPORT SCHEMA (required)
- LOOP_RESULT payload must include `report` object with:
  - `acceptance_matrix`: one row per user-story question. Each row MUST include
    `item`, `status` (`PASS`|`FAIL`|`N/A`), `evidence` (non-empty), `critical`
    (bool), `confidence` (0.0–1.0), and `evidence_strength` (`LOW`|`MEDIUM`|`HIGH`).
    Mark `critical: true` for stories whose failure blocks the checkpoint's core
    value. A critical story with `confidence < 0.75` or `evidence_strength == LOW`
    forces a FAIL controlled transition.
  - `top_findings`: max 3 product insights, each with `impact`
    (`BLOCKER`|`MAJOR`|`MINOR`|`QUESTION`), `title`, `evidence`, `action`; ordered
    by impact priority (BLOCKER→QUESTION).
  - `state_transition`: `before` and `after` objects `{stage, checkpoint, status}`;
    `after.status` MUST mirror the top-level LOOP_RESULT status (PASS keeps
    `IN_REVIEW`; FAIL moves to `IN_PROGRESS`/`BLOCKED`).
  - `loop_result`: exact mirror of the top-level LOOP_RESULT required fields.

LOOP_RESULT (required final line)
Emit exactly one line:
LOOP_RESULT: {"loop":"product_validation","result":"pass|fail","stage":"<id>","checkpoint":"<id>","status":"IN_REVIEW|IN_PROGRESS|BLOCKED","next_role_hint":"review|implement|issues_triage","report":<report_json>}
Then record it with:
`python3 .codex/skills/vibe-loop/scripts/agentctl.py --repo-root . --format json loop-result --line 'LOOP_RESULT: {...,"report":<report_json>}'`

ON PASS: `status` stays `IN_REVIEW`, `result` = "pass", `next_role_hint` = "review".
ON FAIL: set `status` to `IN_PROGRESS` (or `BLOCKED` if a human decision is needed),
`result` = "fail", `next_role_hint` = "implement|issues_triage".

STOP CONDITION
Stop after the UAT session is complete, state is updated, and LOOP_RESULT is
emitted + recorded. On PASS, return control to the dispatcher so it advances to
technical review; on FAIL, the dispatcher routes back to implementation.
```
...
2) Pass B: Adversarial review:
   ...
   - For `[UI]` checkpoints: you MUST physically run the app on the Android emulator to verify UI flows. Tracing code logic is no longer sufficient. Take screenshots as evidence.
3) Pass C: Code review for improvements:
   ...
   - **If ANY code changes are made during this review (even `[MINOR]` fixes), the full test suite (Docker Go tests + Emulator build) MUST be re-run successfully before emitting a PASS verdict.**
...
5) Evaluator-Optimizer pass (required):
   - Score `correctness`, `scope_control`, `evidence_quality`, `state_transition_accuracy` from 1-5.
   - If any score < 5, run one targeted repair/retest pass and rescore once.
   - If any score remains < 5, force FAIL and route to `issues_triage`.
